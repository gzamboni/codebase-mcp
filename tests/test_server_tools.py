...
from yacodebase_mcp.server import mcp
...
def test_search_symbols_no_repos():
    ...
    with patch("yacodebase_mcp.server.get_all_repos", return_value={}):
...
def test_find_todos_finds_comment(tmp_path):
    ...
    fn = _get_tool("find_todos")
    with patch(
        "yacodebase_mcp.server.get_all_repos",
        return_value={
            str(tmp_path): {
                "repo_id": "abc",
...
def test_what_changed_detects_modified(tmp_path):
    ...
    with (
        ...
        patch("yacodebase_mcp.indexer.iter_files", return_value=[src]),
    ):
        result = fn(repo_path=str(tmp_path))
...
def test_search_decisions_tool(tmp_path, monkeypatch):
    monkeypatch.setenv("YACODEBASE_MCP_DATA_DIR", str(tmp_path))
    from yacodebase_mcp.knowledge import add_decision as _add
...
def test_get_notes_tool(tmp_path, monkeypatch):
    monkeypatch.setenv("YACODEBASE_MCP_DATA_DIR", str(tmp_path))
    from yacodebase_mcp.knowledge import add_note as _add
...
def test_session_bootstrap_returns_summary(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("yacodebase_mcp.indexer.iter_files", return_value=[]),
    ):
        result = fn()
